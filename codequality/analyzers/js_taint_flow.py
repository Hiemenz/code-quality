...
def _source_provenance(node, source):
    ...
    if kind == "subscript_expression":
        obj = node.child_by_field_name("object")
        base = _dotted_name(obj, source) if obj is not None else None
...
def _declarator_target_names(declarator, source):
...
def _sink_issue(node, path, tainted, source):
    ...
    return Issue(
        ...
        f".{_text(method, source)}() -- vulnerable to SQL injection"
    )
...
def _walk_stmts(stmts, tainted, issues, path, source):
    for stmt in stmts:
        _walk_stmt(stmt, tainted, issues, path, source)
...
def _walk_declaration(stmt, tainted, issues, path, source):
    for i in range(stmt.named_child_count):
        ...
        prov = _rhs_provenance(value, tainted, source)
        _apply_binding(names, prov, tainted)


def _walk_assignment(expr, tainted, issues, path, source):
...
def _walk_stmt(stmt, tainted, issues, path, source):
    ...
    if kind == "if_statement":
        ...
        if consequence is not None:
            _walk_stmts(_statement_block_children(consequence), then_state, issues, path, source)
    ...
    if kind == "try_statement":
        ...
        body = stmt.child_by_field_name("body")
        if body is not None:
            _walk_stmts(_statement_block_children(body), try_state, issues, path, source)
        ...
        if handler is not None:
            ...
            if handler_body is not None:
                _walk_stmts(_statement_block_children(handler_body), handler_state, issues, path, source)
        ...
        if finalizer_body is not None:
            _walk_stmts(_statement_block_children(finalizer_body), tainted, issues, path, source)
        return
...
def taint_issues(root, path, source, only_lines):
    ...
    for kind in ("function_declaration", "function_expression", "arrow_function", "method_definition"):
        for fn in _iter_kind(root, kind):
            _analyze_one(fn, path, source, issues)
    return [i for i in issues if _in_scope(i.line, only_lines)]


def _analyze_one(fn, path, source, issues):
    local_issues = []
    _walk_stmts(_function_body_stmts(fn), {}, local_issues, path, source)
