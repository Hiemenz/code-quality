"""Groups security-category findings by CWE/OWASP for `codequality compliance`.
...
report total always reconciles with the number of security issues found.
"""
...
def build_report(issues):
    ...
    "unmapped": [issue, ...], "total": int} -- only `category == "security"`
    issues are considered; everything else is ignored.
    """
...
def render_text(report):
    ...
    lines = [f"Compliance Report ({report['total']} security finding(s))", ""]
